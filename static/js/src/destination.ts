...
function addErrorTitle(errormessage:string):void {
...
document.getElementById("formDestination").addEventListener("submit", (e: Event) => {
    e.preventDefault();
    const destinationTitle = (document.getElementById("destination_title") as HTMLInputElement).value;
...
document.getElementById("formDestination").addEventListener("submit", (e: Event) => {
    ...
    (destinationLocation.length > 50) {
    addErrorLocation("Location cannot exceed 90 characters.");
    }
})

/////////////// CONFIRMATION DIALOG FOR DELETION MODAL FUNCTIONALITY ///////////////
function toggleModal(pk: string): void {
    const modal = document.querySelector<HTMLElement>(`#modal-${pk}`);
    if (modal) {
